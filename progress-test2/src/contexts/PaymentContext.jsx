...
export const PaymentProvider = ({ children }) => {
  ...
  const addPayment = async newPayment => {
    const res = await axios.post('http://localhost:3001/payments', newPayment);
    setPayments([...payments, res.data]);
  };

  const updatePayment = async (id, updatedPayment) => {
    const res = await axios.put(`http://localhost:3001/payments/${id}`, updatedPayment);
  ...
  const filteredPayments = useMemo(() => {
    ...
    // 1️⃣ Tìm kiếm theo semester hoặc courseName
    if (searchTerm.trim() !== '') {
      result = result.filter(p =>
        `${p.semester} ${p.courseName}`.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    // 2️⃣ Lọc theo semester
    if (filterSemester) {
      result = result.filter(p => p.semester === filterSemester);
    }

    // 3️⃣ Lọc theo courseName
    if (filterCourse) {
